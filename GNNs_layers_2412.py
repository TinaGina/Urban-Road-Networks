...
def save_csv(list1, csv_filename):
    with open(csv_filename + 'Acc.csv', 'a+', newline='') as csvfile:
            ...
            for city_j in range(city_i + 1, CLASS):  # 从基类以后进行二分类
                ...
                for i, model in enumerate(models):
                    ...
                    for epoch in range(EPOCH):
                        epoch_train_acc, epoch_test_acc = fit(epoch, model, data_train, data_test)
                        train_acc.append(epoch_train_acc)
                        test_acc.append(epoch_test_acc)
                    save_csv(train_acc, pwd + 'num_layers' + num_layers + names[i] + 'Train')
                    save_csv(test_acc, pwd +  'num_layers' + num_layers + names[i] + 'Test')
